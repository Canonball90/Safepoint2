...
public class ConfigInitializer {
    ...
    public void init() {
        ...
        if (!path.exists())
            path.mkdir();
        modules.addAll(Safepoint.moduleInitializer.getModuleList());
        load();
    }
    ...
    public void saveFriendList() {
        try {
            ...
            for (FriendInitializer.FriendPlayer friendPlayer : Safepoint.friendInitializer.getFriendList()) {
    ...
    public void loadFriendList() {
        try {
            ...
            bufferReader.lines().forEach(line -> {
                String name = line;
                Safepoint.friendInitializer.addFriend(name);
            });
        } catch (Exception ignored) {
        }
    }
    ...
    public void saveHudState() {
        try {
            for (HudModule hudModule : Safepoint.hudComponentInitializer.getHudModules()) {
